...
void AXP192::begin()
{
    ...
    //AXP192 30H
    Writeuint8(0x30, (Read8bit(0x30) & 0x04) | 0X02);
    Serial.printf("axp: vbus limit off\n");

    //AXP192 GPIO1:OD OUTPUT
    Writeuint8(0x92, Read8bit(0x92) & 0xf8);
    Serial.printf("axp: gpio1 init\n");

    //AXP192 GPIO2:OD OUTPUT
    Writeuint8(0x93, Read8bit(0x93) & 0xf8);
    Serial.printf("axp: gpio2 init\n");

    //AXP192 RTC CHG
    Writeuint8(0x35, (Read8bit(0x35) & 0x1c) | 0xa2);
    ...
    //AXP192 GPIO4
    Writeuint8(0X95, (Read8bit(0x95) & 0x72) | 0X84);

    Writeuint8(0X36, 0X4C);

    Writeuint8(0x82,0xff);
    ...
    if(Read8bit(0x00) & 0x08) {
        Writeuint8(0x30, Read8bit(0x30) | 0x80);
...
void AXP192::Writeuint8(uint8_t Addr, uint8_t Data)
{
...
void AXP192::ScreenBreath(uint8_t brightness)
{
    ...
    Writeuint8(0x28, ((buf & 0x0f) | (brightness << 4)));
}
...
{
    Writeuint8(0x32, Read8bit(0x32) | 0b10000000);
}

void AXP192::SetAdcState(bool state)
{
    // Enable / Disable all ADCs
    Writeuint8(0x82, state ? 0xff : 0x00);
}
...
void AXP192::SetLDOVoltage(uint8_t number, uint16_t voltage)
{
    ...
    //uint8_t reg, data;
    case 2:
        Writeuint8(0x28, (Read8bit(0x28) & 0X0F) | (voltage << 4));
        break;
    case 3:
        Writeuint8(0x28, (Read8bit(0x28) & 0XF0) | voltage);
        break;
    }
}
...
void AXP192::SetDCVoltage(uint8_t number, uint16_t voltage)
{
    ...
    Writeuint8(addr, (Read8bit(addr) & 0X80) | (voltage & 0X7F));
}
...
void AXP192::SetLDOEnable(uint8_t number, bool state)
{
    ...
    if (state)
    {
        Writeuint8(0x12, (Read8bit(0x12) | mark));
    }
    else
    {
        Writeuint8(0x12, (Read8bit(0x12) & (~mark)));
...
void AXP192::SetBusPowerMode(uint8_t state)
{
    ...
    if (state == 0)
    {
        ...
        data = Read8bit(0x91);
        Writeuint8(0x91, (data & 0x0F) | 0xF0);
        // Set GPIO0 to LDO OUTPUT, pullup N_VBUSEN to disable VBUS supply from BUS_5V
        data = Read8bit(0x90);
        Writeuint8(0x90, (data & 0xF8) | 0x02);
        // Set EXTEN to enable 5v boost
        data = Read8bit(0x10);
        Writeuint8(0x10, data | 0x04);
    }
    else
    {
        ...
        data = Read8bit(0x90);
        Writeuint8(0x90, (data & 0xF8) | 0x07);
    }
}
