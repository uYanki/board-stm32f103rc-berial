/*
** $Id: bitop.h,v 1.5 2004/02/25 16:07:02 jh8 Exp $
**
** bitopt.c
**
...
typedef struct _BITOP {
    uint8_t* pucBitBuffer;
    uint32_t uiBitBufferSize;
    uint32_t uiMaxBits;
} BITOP;
...
**  @return !0  failed
*/
static INLINE int boInitStaticBITOP(BITOP* BitOp, int iBytes, uint8_t* buf)
{
    if (iBytes < 1 || !buf || !BitOp)
        return 1;

    BitOp->pucBitBuffer    = buf;
    BitOp->uiBitBufferSize = (uint32_t)iBytes;
    BitOp->uiMaxBits       = (uint32_t)(iBytes << 3);
...
static INLINE int boInitBITOP(BITOP* BitOp, int iSize)
{
    ...
    */
    ...
    BitOp->pucBitBuffer = (uint8_t*)calloc(1, iBytes);
    if (BitOp->pucBitBuffer == NULL) {
        return 1;
    }

    BitOp->uiBitBufferSize = (uint32_t)iBytes;
    BitOp->uiMaxBits       = (uint32_t)iSize;
...
**
*/
static INLINE int boSetBit(BITOP* BitOp, uint32_t uiPos)
{
    uint8_t mask;
...
**
*/
static INLINE int boIsBitSet(BITOP* BitOp, uint32_t uiPos)
{
    uint8_t mask;
...
**
*/
static INLINE void boClearBit(BITOP* BitOp, uint32_t uiPos)
{
    uint8_t mask;
...
**
*/
static INLINE void boClearByte(BITOP* BitOp, uint32_t uiPos)
{
    /*
