...
#if 1
#include "i2cdev_conf.h"
#else
#include "i2cdev/i2cdev_conf.h"
#endif
...
typedef struct hard_i2c {
    // i2c
    I2C_TypeDef* I2C;

    bool RemapIO;  // for I2C1

    uint32_t Speed;  // max = 400k
    uint32_t Timeout;
    uint16_t OwnAddr;  // self addr
}* hard_i2c_t;
...
/* examples:
    ...
    struct soft_i2c_t  swi2c_bus;
    struct i2c_cli     bme280;

    swi2c_bus.Clock    = RCC_APB2Periph_GPIOB;
