...
bool mlx90614_read_word_crc(uint8_t reg, uint16_t* data)
{
    ...
    if (i2cdev_read_block(&mlx90614, reg, buff, 3)) {
        lsb     = buff[0];
        msb     = buff[1];
        pec     = buff[2];
        // check crc
        buff[0] = mlx90614.dev;    // set read addr
        buff[1] = reg;             // set regsiter
        buff[2] = buff[0] | 0x01;  // set write addr
        buff[3] = lsb;
        buff[4] = msb;
        if (calc_crc(buff, 5) == pec) {
            // read success
            *data = (msb << 8) | lsb;
            return true;
        }
...
bool mlx90614_write_word_crc(uint8_t reg, uint16_t data)
{
    ...
    buff[3] = data >> 8;    // msb
    buff[4] = calc_crc(buff, 4);

    return i2cdev_write_block(&mlx90614, reg, &buff[2], 3);
}

bool mlx90614_read_bit_crc(uint8_t reg, uint8_t bit, uint8_t* data)
{
    uint16_t buff;
    if (mlx90614_read_word_crc(reg, &buff)) {
        *data = (buff >> bit) & 0x01;
        return true;
    } else {
        *data = 0;
...
bool mlx90614_write_eeprom(uint8_t reg, uint16_t data)
{
    ...
    if (mlx90614_write_word_crc(reg, 0x00)) {
        // delay tErase at least 5 ms
        HAL_Delay(10);
        if (mlx90614_write_word_crc(reg, data)) {
            // delay tErase at least 5 ms
            HAL_Delay(10);
            return true;
        }
...
{
    return mlx90614_read_word_crc(MLX90614_REG_RAWIR1, data);
}

bool mlx90614_get_raw_ir_channel_2(uint16_t* data)
{
    return mlx90614_read_word_crc(MLX90614_REG_RAWIR2, data);
}

uint8_t mlx90614_get_flag(void)
{
    uint16_t buff;
    return mlx90614_read_word_crc(COMMAND_READ_FLAGS, &buff) ? buff : 0x00;
}
...
bool mlx90614_enter_sleep_mode(void)
{
    uint8_t buff[2], crc;
    buff[0] = mlx90614.dev;
    buff[1] = COMMAND_ENTER_SLEEP;
    return i2cdev_write_byte(&mlx90614, COMMAND_ENTER_SLEEP, calc_crc(buff, 2));
}
...
{
    return mlx90614_read_word_crc(MLX90614_REG_TOBJ1, (uint16_t*)raw) && ((*raw & 0x8000) == 0) /* check error flag */;
}

bool mlx90614_get_object2_temp(int16_t* raw)
{
    return mlx90614_read_word_crc(MLX90614_REG_TOBJ2, (uint16_t*)raw) && ((*raw & 0x8000) == 0) /* check error flag */;
}

bool mlx90614_get_ambient_temp(int16_t* raw)
{
    return mlx90614_read_word_crc(MLX90614_REG_TA, (uint16_t*)raw);
}

bool mlx90614_get_min_temp(int16_t* raw)
{
    return mlx90614_read_word_crc(MLX90614_REG_TO_MIN, (uint16_t*)raw);
}

bool mlx90614_get_max_temp(int16_t* raw)
{
    return mlx90614_read_word_crc(MLX90614_REG_TO_MAX, (uint16_t*)raw);
}
...
{
    return mlx90614_read_word_crc(MLX90614_REG_ID1, &ID[0]) &&
           mlx90614_read_word_crc(MLX90614_REG_ID2, &ID[1]) &&
           mlx90614_read_word_crc(MLX90614_REG_ID3, &ID[2]) &&
           mlx90614_read_word_crc(MLX90614_REG_ID4, &ID[3]);
}

bool mlx90614_get_emissivity(float* emissivity)
{
    uint16_t buff;
    if (mlx90614_read_word_crc(MLX90614_REG_EMISSIVITY, &buff)) {
...
{
    uint16_t buff;
    if (mlx90614_read_word_crc(MLX90614_REG_ADDRESS, &buff)) {
