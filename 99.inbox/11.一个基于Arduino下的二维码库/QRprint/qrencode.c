...
extern uint8_t neccblk1;
extern uint8_t neccblk2;
extern uint8_t datablkw;
extern uint8_t eccblkwid;
extern uint8_t VERSION;
extern uint8_t ECCLEVEL;
extern uint8_t WD, WDB;
#ifndef USEPRECALC
// These are malloced by initframe
extern uint8_t* rlens;
extern uint8_t* framebase;
extern uint8_t* framask;
#else
extern uint8_t rlens[];
extern uint8_t framebase[] PROGMEM;
extern uint8_t framask[] PROGMEM;
#endif

//========================================================================
// Reed Solomon error correction
static uint32_t modnn(uint32_t x)
{
    while (x >= 255) {
        x -= 255;
        x = (x >> 8) + (x & 255);
...
#ifndef RTGENEXPLOG
static const uint8_t g0log[256] PROGMEM = {
...
static const uint8_t g0exp[256] PROGMEM = {
...
static void initrspoly(uint8_t eclen, uint8_t* genpoly)
{
    uint8_t i, j;
...
static void appendrs(uint8_t* data, uint8_t dlen, uint8_t* ecbuf, uint8_t eclen, uint8_t* genpoly)
{
    uint8_t i, j, fb;
...
static void stringtoqr(void)
{
    ...
    uint8_t* ecc = &strinbuf[max];
    uint8_t* dat = strinbuf;
...
static uint8_t ismasked(uint8_t x, uint8_t y)
{
    uint32_t bt;
    if (x > y) {
        bt = x;
...
static const uint8_t N1 = 3;
static const uint8_t N2 = 3;
static const uint8_t N3 = 40;
static const uint8_t N4 = 10;

static uint32_t badruns(uint8_t length)
{
    uint8_t  i;
    uint32_t runsbad = 0;
    for (i = 0; i <= length; i++)
...
// level << 3 | mask
static const uint32_t fmtword[] PROGMEM = {
...
static void addfmt(uint8_t masknum)
{
    uint32_t fmtbits;
    uint8_t  i, lvl = ECCLEVEL - 1;
