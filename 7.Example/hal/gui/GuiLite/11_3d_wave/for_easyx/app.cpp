...
void create_ui(void* phy_fb, int screen_width, int screen_height, int color_bytes, struct DISPLAY_DRIVER* driver)
{
    ...
    while (1) {
        BeginBatchDraw();
        theCwave.draw(30 + UI_WIDTH / 2, 10 + UI_HEIGHT / 2, true);  // erase footprint
        ...
        theCwave.draw(30 + UI_WIDTH / 2, 10 + UI_HEIGHT / 2, false);  // refresh Cwave
        thread_sleep(25);
        step++;
        EndBatchDraw();
    }
...
extern "C" void startHello3Dwave(void* phy_fb, int color_bytes, struct DISPLAY_DRIVER* driver)
{
...
void gfx_fill_rect(int x0, int y0, int x1, int y1, uint32_t rgb)
{
    if (x0 > x1) return;
    if (y0 > y1) return;
    setfillcolor(gl_color_to_easyx_color(rgb));
    solidrectangle(x0, y0, x1, y1);
}
...
int main()
{
    ...
    startHello3Dwave(NULL, 4, &driver);
    closegraph();
