...
#define xmalloc             malloc /* or something that exits if not enough memory */
#define xfree               free
#endif

#define countof(x) (sizeof(x) / sizeof((x)[0]))
#define TOLOWER(x) ('A' <= (x) && (x) <= 'Z' ? (x) + 32 : (x)) /* xinz  Modify */
#define PARAMS(x)  x
...
typedef uint32_t(*hashfun_t) PARAMS((const void*));
...
#define INVALID_PTR ((void*)~(uint32_t)0)
#ifndef UCHAR_MAX
...
void hash_table_map(struct hash_table* ht,
...
int print_hash_table_mapper(void* key, void* value, void* count)
{
    ++*(int*)count;
