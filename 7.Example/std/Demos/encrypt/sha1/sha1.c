...
void sha1(uint8_t* src, int srclen, uint8_t* des)
{
    ...
    SHA1Init(&context);
    SHA1Update(&context, src, srclen);
    SHA1Final(des, &context);
}
...
void sha1s(uint8_t* src, int srclen, char* des)
{
    ...
    sha1(src, srclen, sha1_d);

    for (i = 0; i < 20; i++) {
        *des = XNUM_TO_digit(sha1_d[i] >> 4);
        des++;
        *des = XNUM_TO_digit(sha1_d[i] & 0xf);
        des++;
    }
...
int main(int argc, char** argv)
{
    ...
    {
        const char* str = "abc";
        sha1s((uint8_t*)str, strlen(str), buffer);
        printf("%s\n", buffer);
    }

    {
        const char* str = "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq";
        sha1s((uint8_t*)str, strlen(str), buffer);
        printf("%s\n", buffer);
    }
    ...
    if (argc < 2) {
        // 从控制台读取，输入完成按 "Ctrl+Z"
        printf("read data from console:\n");
        file = stdin;
    } else if (argc == 2) {
        // 从文件读取，需执行指令 "./sha1.exe test.txt"
