...
uint8_t set_freq(uint64_t freq, enum si5351_clock clk)
{
    ...
    if ((uint8_t)clk <= (uint8_t)SI5351_CLK5) {
        ...
        if (freq == 0) {
            // pass
        } else if (freq < SI5351_CLKOUT_MIN_FREQ) {
        ...
        if (freq > (SI5351_MULTISYNTH_SHARE_MAX)) {
            ...
            uint8_t i;
            for (i = 0; i < 6; i++) {
                if (clk_freq[i] > (SI5351_MULTISYNTH_SHARE_MAX)) {
            ...
            for (i = 0; i < 6; i++) {
                if (clk_freq[i] != 0) {
                    if (pll_assignment[i] == pll_assignment[clk]) {
                        ...
                        if (temp_freq >= SI5351_MULTISYNTH_DIVBY4_FREQ) {
                            div_by_4 = 1;
                            int_mode = 1;
                        } else {
                            div_by_4 = 0;
        ...
        // Lower bounds check
        if (freq > 0 && freq < SI5351_CLKOUT67_MIN_FREQ) {
            freq = SI5351_CLKOUT_MIN_FREQ;
        }

        // Upper bounds check
        if (freq >= SI5351_MULTISYNTH_DIVBY4_FREQ) {
            freq = SI5351_MULTISYNTH_DIVBY4_FREQ - 1;
        }
...
uint8_t set_freq_manual(uint64_t freq, uint64_t pll_freq, enum si5351_clock clk)
{
    ...
    if (freq == 0) {
        ...
        freq = SI5351_CLKOUT_MIN_FREQ;
    } else if (freq > SI5351_CLKOUT_MAX_FREQ) {
        freq = SI5351_CLKOUT_MAX_FREQ;
    }
...
void set_pll(uint64_t pll_freq, enum si5351_pll target_pll)
{
    ...
    params[1] = pll_reg.p3 & 0xFF;
    params[2] = (pll_reg.p1 >> 16) & 0x03;
    params[3] = (pll_reg.p1 >> 8) & 0xFF;
    params[4] = pll_reg.p1 & 0xFF;
    params[5] = ((pll_reg.p3 >> 12) & 0xF0) + ((pll_reg.p2 >> 16) & 0x0F);
    params[6] = (pll_reg.p2 >> 8) & 0xFF;
    params[7] = pll_reg.p2 & 0xFF;
...
{
    static const uint8_t regs[] = {42, 50, 58, 66, 74, 82, 90, 91};
    ...
    switch (clk) {
        ...
        case SI5351_CLK5:
            ...
            uint8_t params[8];
            params[0] = (ms_reg.p3 >> 8) & 0xFF;
            params[1] = ms_reg.p3 & 0xFF;
            params[2] = (ms_reg.p1 >> 16) & 0x03;
            params[3] = (ms_reg.p1 >> 8) & 0xFF;
            params[4] = ms_reg.p1 & 0xFF;
            params[5] = ((ms_reg.p3 >> 12) & 0xF0) + ((ms_reg.p2 >> 16) & 0x0F);
            params[6] = (ms_reg.p2 >> 8) & 0xFF;
            params[7] = ms_reg.p2 & 0xFF;

            params[2] |= r_div << 4;

            if (div_by_4) params[2] |= 3 << 2;

            si5351_write_bytes(regs[clk], params, 8);
        ...
        case SI5351_CLK7:
            ...
            si5351_write(regs[clk], ms_reg.p1);

            switch (clk) {
                case SI5351_CLK6:
                    si5351_write_bits(92, 0, 3, r_div);
                    break;
                case SI5351_CLK7:
                    si5351_write_bits(92, 4, 3, r_div);
                    break;
            }
...
{
    si5351_write_bit(SI5351_OUTPUT_ENABLE_CTRL, clk, enable ? 0 : 1);
}
...
{
    si5351_write_bits(SI5351_CLK0_CTRL + (uint8_t)clk, 0, 2, drive);
}
...
{
    si5351_write(SI5351_CLK0_PHASE_OFFSET + (uint8_t)clk, phase & 0x7F);
}
 ...
 */
void set_int(enum si5351_clock clk, si5351_ms_mode_t mode)
{
    si5351_write_bit(SI5351_CLK0_CTRL + (uint8_t)clk, 6, (uint8_t)mode);
}
...
void set_vcxo(uint64_t pll_freq, uint8_t ppm)
{
    ...
    } else if (ppm > SI5351_VCXO_PULL_MAX) {
    ...
    si5351_write_bytes(SI5351_PLLB_PARAMETERS, params, 8);

    // Write the VCXO parameters
    vcxo_param = ((vcxo_param * ppm * SI5351_VCXO_MARGIN) / 100ULL) / 1000000ULL;

    params[0] = vcxo_param & 0xFF;
    params[1] = (vcxo_param >> 8) & 0xFF;
    params[2] = (vcxo_param >> 16) & 0x3F;
    si5351_write_bytes(SI5351_VXCO_PARAMETERS_LOW, params, 3);
}
...
uint8_t si5351_write_bulk(uint8_t reg, uint8_t cnt, uint8_t* dat)
{
    si5351_write_bytes(reg, dat, cnt);
}

uint8_t si5351_write(uint8_t reg, uint8_t dat)
{
    si5351_write_uint8(reg, dat);
}

uint8_t si5351_read(uint8_t addr)
{
    return si5351_read_uint8(reg);
}
...
uint64_t pll_calc(enum si5351_pll pll, uint64_t freq, struct Si5351RegSet* reg, int32_t correction, uint8_t vcxo)
{
    uint64_t ref_freq;
    if (pll == SI5351_PLLA) {
        ref_freq = xtal_freq[(uint8_t)plla_ref_osc];
    } else {
        ref_freq = xtal_freq[(uint8_t)pllb_ref_osc];
    }
    // ref_freq = 15974400ULL ;
    ...
    if (vcxo) {
        return (uint64_t)((a << 7) * 1000000ULL + b);
    } else {
        return freq;
    }
}
...
uint64_t multisynth_calc(uint64_t freq, uint64_t pll_freq, struct Si5351RegSet* reg)
{
    ...
    if (pll_freq == 0) {
        ...
        if (divby4 == 0) {
            lltmp = SI5351_PLL_VCO_MAX;  // margin needed?
            do_div(lltmp, freq);
            if (lltmp == 5) {
                lltmp = 4;
            } else if (lltmp == 7) {
...
uint64_t multisynth67_calc(uint64_t freq, uint64_t pll_freq, struct Si5351RegSet* reg)
{
    ...
    // Multisynth bounds checking
    if (freq > SI5351_MULTISYNTH67_MAX_FREQ) {
        freq = SI5351_MULTISYNTH67_MAX_FREQ;
    } else if (freq < SI5351_MULTISYNTH_MIN_FREQ) {
        freq = SI5351_MULTISYNTH_MIN_FREQ;
    }
    ...
    if (pll_freq == 0) {
        // Find largest integer divider for max
        // VCO frequency and given target frequency
        lltmp = (SI5351_PLL_VCO_MAX)-100000000UL;  // margin needed?
        do_div(lltmp, freq);
        a = (uint32_t)lltmp;
        ...
        } else if (a > SI5351_MULTISYNTH67_A_MAX) {
...
uint8_t select_r_div(uint64_t* freq)
{
    ...
    // SI5351_CLKOUT_MIN_FREQ = 4000 Hz

    if (*freq <= 4000ULL && *freq < 8000ULL) {  // < 8kHz
        r_div = SI5351_CLKOUT_DIV_128;
    } else if (*freq < 16000ULL) {  // 8kHz ~ 16kHz
        r_div = SI5351_CLKOUT_DIV_64;
    } else if (*freq < 32000ULL) {  // 16kHz ~ 32kHz
        r_div = SI5351_CLKOUT_DIV_32;
    } else if (*freq < 64000ULL) {  // 32kHz ~ 64kHz
        r_div = SI5351_CLKOUT_DIV_16;
    } else if (*freq < 128000ULL) {  // 64kHz ~ 128kHz
        r_div = SI5351_CLKOUT_DIV_8;
    } else if (*freq < 256000ULL) {  // 128kHz ~ 256kHz
        r_div = SI5351_CLKOUT_DIV_4;
    } else if (*freq < 512000ULL) {  // 256kHz ~ 512kHz
        r_div = SI5351_CLKOUT_DIV_2;
    } else {  // > 512kHz
        r_div = SI5351_CLKOUT_DIV_1;
    }
...
uint8_t select_r_div_ms67(uint64_t* freq)
{
    ...
    if ((*freq >= SI5351_CLKOUT67_MIN_FREQ) && (*freq < SI5351_CLKOUT67_MIN_FREQ * 2)) {
        r_div = SI5351_CLKOUT_DIV_128;
    } else if (*freq < SI5351_CLKOUT67_MIN_FREQ * 4) {
        r_div = SI5351_CLKOUT_DIV_64;
    } else if (*freq < SI5351_CLKOUT67_MIN_FREQ * 8) {
        r_div = SI5351_CLKOUT_DIV_32;
    } else if (*freq < SI5351_CLKOUT67_MIN_FREQ * 16) {
        r_div = SI5351_CLKOUT_DIV_16;
    } else if (*freq < SI5351_CLKOUT67_MIN_FREQ * 32) {
        r_div = SI5351_CLKOUT_DIV_8;
    } else if (*freq < SI5351_CLKOUT67_MIN_FREQ * 64) {
        r_div = SI5351_CLKOUT_DIV_4;
    } else if (*freq < SI5351_CLKOUT67_MIN_FREQ * 128) {
        r_div = SI5351_CLKOUT_DIV_2;
    } else {
        r_div = SI5351_CLKOUT_DIV_1;
    }
